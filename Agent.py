...
class CreditAssignment:
    ...
    def _normalize(
        self, s_start: float, s_end: float, h_start: float
    ) -> Tuple[float, float]:
    ...
    def show_dist(self, s_start: float, s_end: float, h_start: float):
        s_norm_start, s_norm_end = self._normalize(s_start, s_end, h_start)
        x = np.linspace(self.dist.ppf(0.01), self.dist.ppf(0.99))
        plt.plot(x, self.dist.pdf(x), "r-")
        plt.vlines(
            s_norm_start, ymin=0, ymax=self.dist.pdf(s_norm_start), color="green"
        )
...
class NetworkController(PyGymCallback):
    ...
    def __init__(
        self, encoder, head, queue, img_dims=(3, 160, 160), ts_len=0.3, **kwargs
    ):
    ...
    def before_set_action(self):
        ...
        state = self.env.render().transpose((2, 0, 1))
        state = np.ascontiguousarray(state, dtype=np.float32) / 255
        state = torch.from_numpy(state)
        resize = T.Compose(
            [T.ToPILImage(), T.Resize((self.img_dims[1:])), T.ToTensor()]
        )
    ...
    def after_step(self):
        ...
        if fb != 0:
            ...
            for win in self.sliding_window:
                credit_for_state = ca(
                    s_start=win["s_start"], s_end=win["s_end"], h_start=h_time
                )
...
class FeedbackListener(Process):
    ...
    def run(self, fps=60):
        ...
        while self.listening:
            ...
            self.clock.tick(fps)
            self.fb_queue.put(dict(feedback=fb, h_time=time.time()))
...
def main():
    env = gym.make("Bowling-v0", render_mode="rgb_array").unwrapped
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    encoder = Encoder().to(device)
    head_net = Head().to(device)
    encoder.load_state_dict(
        torch.load("auto_encoder/Type_1/encoder.pt", map_location=device)
    )
